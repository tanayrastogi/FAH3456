...
class AdjBaseModel:
    ...
    # Nested Probability
    def probability_function(self, V, lmbda):
        # Denominator
        den = list()
        for i in range(len(model.tree)):
            node = model.tree[i]
            den.append(np.power(np.nansum(np.exp(V[:, node]/lmbda[i]), axis=1), lmbda[i]))
        den = sum(den).reshape(-1, 1)
        # Numerator
        num = np.zeros(V.shape)
        for i in range(len(model.tree)):
            node = model.tree[i]
            summ = np.power(np.nansum(np.exp(V[:, node]/lmbda[i]), axis=1), lmbda[i] - 1)
            for j in node:
                num[:, j] = (np.exp(V[:, j]/lmbda[i]))*summ

        Pr = np.divide(num, den)
    ...
    def optimization_function(self, parameters):
        ...
        V = V/100
        # Probability
        Pr = self.probability_function(V, lmbda)
        # Loglikelihood
...
if __name__ == "__main__":
    ...
    # V = V/100  
    # # Probability
    # Pr = model.probability_function(V, lmbda)
    # # LL
